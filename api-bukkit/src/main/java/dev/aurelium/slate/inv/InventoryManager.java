...
public class InventoryManager {
    ...
    private final Map<UUID, PlayerInvTask> updateTasks;
...
//        new InvTask().runTaskTimer(plugin, 1, 1);
    ...
    protected void cancelUpdateTask(Player p) {
    	if(updateTasks.containsKey(p.getUniqueId())) {
          //int bukkitTaskId = this.updateTasks.get(p.getUniqueId()).getTaskId();
            PlayerInvTask task = this.updateTasks.get(p.getUniqueId());
    ...
    class InvTask extends BukkitRunnableSlate {
    ...
    class PlayerInvTask extends BukkitRunnableSlate {
    ...
    class BukkitRunnableSlate {

        ScheduledTask thetaks;

        public void run() {}

        public BukkitRunnableSlate runTaskTimer(Plugin plugin, long delay, long period){
            long delayaux = 1;
            long periodaux = 1;
            if((TimeUnit.MILLISECONDS.toMillis(delay) / 50) > 0){
                delayaux = TimeUnit.MILLISECONDS.toMillis(delay) / 50;
            }
            if((TimeUnit.MILLISECONDS.toMillis(period) / 50) > 0){
                periodaux = TimeUnit.MILLISECONDS.toMillis(period) / 50;
            }
            thetaks = plugin.getServer().getGlobalRegionScheduler().runAtFixedRate(plugin, task -> { this.run(); }, delayaux, periodaux);
            return this;
        }
